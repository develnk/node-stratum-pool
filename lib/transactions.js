...
function scriptCompile(addrHash){
    script = bitGoUTXO.script.compile(
        [
            bitGoUTXO.opcodes.OP_DUP,
            bitGoUTXO.opcodes.OP_HASH160,
            addrHash,
            bitGoUTXO.opcodes.OP_EQUALVERIFY,
            bitGoUTXO.opcodes.OP_CHECKSIG
        ]);
    return script;
}
...
exports.createGeneration = function(blockHeight, blockReward, feeReward, recipients, poolAddress, payFoundersReward, percentFoundersReward, maxFoundersRewardBlockHeight, foundersRewardAddressChangeInterval, vFoundersRewardAddress, percentTreasuryReward, treasuryRewardStartBlockHeight, treasuryRewardAddressChangeInterval, vTreasuryRewardAddress){
    var poolAddrHash = bitGoUTXO.address.fromBase58Check(poolAddress).hash;
    var tx = new bitGoUTXO.Transaction();
    ...
    if (payFoundersReward === true && (maxFoundersRewardBlockHeight >= blockHeight || treasuryRewardStartBlockHeight)) {
        ...
        if (treasuryRewardStartBlockHeight && blockHeight >= treasuryRewardStartBlockHeight) {
            ...
            var foundersAddrHash = bitGoUTXO.address.fromBase58Check(vTreasuryRewardAddress[index]).hash;
            ...
            tx.addOutput(
                ...
                (Math.round(blockReward * (1 - (percentTreasuryReward + feePercent) / 100))) * 100
            );
            // treasury t-addr
            tx.addOutput(
                scriptFoundersCompile(foundersAddrHash),
                (Math.round(blockReward * (percentTreasuryReward / 100))) * 100
            );
            ...
            var foundersAddrHash = bitGoUTXO.address.fromBase58Check(vFoundersRewardAddress[index]).hash;
            ...
            tx.addOutput(
                scriptCompile(poolAddrHash),
                (Math.round(blockReward * (1 - (percentFoundersReward + feePercent) / 100))) * 100
            );
            // founders t-addr
            tx.addOutput(
                scriptFoundersCompile(foundersAddrHash),
                (Math.round(blockReward * (percentFoundersReward / 100))) * 100
            );
        }
    }
    // no founders rewards :)
    else
    {
        ...
        tx.addOutput(
            scriptCompile(poolAddrHash),
            (Math.round(blockReward * (1 - (feePercent / 100)))) * 100
        ); 
    }
    ...
    for (var i = 0; i < recipients.length; i++) {
       tx.addOutput(
           scriptCompile(bitGoUTXO.address.fromBase58Check(recipients[i].address).hash),
           (Math.round(blockReward * (recipients[i].percent / 100))) * 100
       );
    }
